...
$$\mathbf{z}_k = (1-\alpha) \cdot \text{Conv}_{lo}(\mathbf{x}) + \alpha \cdot \text{Conv}_{hi}(\mathbf{x})$$

由于 $\partial\alpha/\partial s = 1$，梯度为 $\partial L/\partial s = \partial L/\partial\mathbf{z}_k \cdot (\text{Conv}_{hi} - \text{Conv}_{lo})$。lo/hi 的取整仅用于索引预创建的卷积核，不参与计算图。
...
温度退火策略：τ(t) = max(0.5, 5.0 × (1 − t/T))。训练初期 τ=5，分布较平，模型在各候选间探索；训练末期 τ=0.5，分布逼近 one-hot，与推理行为一致。
