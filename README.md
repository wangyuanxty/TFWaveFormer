...
TFWaveFormer 的论文在 Section 6 提了一个很有意思的方向——作者在实验中发现不同数据集上最优的小波尺度不太一样：
...
大意是不同图的最优小波尺度应该不同，如果能根据图特征自适应配置就好了。陈老师组里的这篇工作在我想象中是挺扎实的。原版用的核尺寸是固定的 `[3, 5, 7, 9]`，挺合理的。不过不同的图确实有不太一样的时序特点，比如交互很密集的图可能需要小一点的窗口看局部细节，交互周期长的图可能需要大一点的窗口看长程趋势。如果能把这个选择交给模型自己决定，可能会更方便。

然后遇到一个问题：Conv1d 的 `kernel_size` 得是奇数整数，而取整函数本身不可导。所以没办法直接把 `kernel_size` 设置成可学习参数——梯度回不来。
...
参考了 Dynamic Filter Networks (NIPS 2016)，https://arxiv.org/abs/1605.09673。那篇工作的思路是用小网络根据输入动态生成整个卷积核权重，我这里是简化成只学一个实数尺度。
...
对某个尺度 s，先确定取多少个点（round(s) 调成奇数，这步梯度不参与），比如 7 个点，坐标均匀分布在 [-1,1]。对 172 个通道每个都这样取点、查 MLP、Softmax 归一化，得到 172 组核权重，然后用 `F.conv1d(groups=172)` 完成卷积。梯度通过 MLP 的链式法则回传，s 是 MLP 的普通输入，不受取整影响。
...
挑的过程怎么能求导？Gumbel-Softmax 可以。每个头维护一个 7 维的 logit 向量。训练的时候加 Gumbel 噪声，除温度 τ，做 softmax，得到的是一个概率分布而不是 one-hot。7 个候选卷积全算好，按概率加权加在一起，梯度能过。推理的时候不需要噪声了，直接 argmax 取最大 logit 对应到候选集里，就是最终选的核。

温度和退火是跟 DARTS 学的——一开始 τ 大一点（比如 5.0），分布比较平，模型在各个候选之间试探；训练过程里 τ 慢慢降下来（比如降到 0.5），分布越来越尖，到后期软选择已经和硬选择差不多了：$\tau(t) = \max(0.5, 5.0 \times (1 - t/T))$。
...
目前只跑了 3 epochs × 1 run 做验证，主要是看看这三条路线能不能正常跑通。超参通过 `--load_best_configs` 自动加载，自适应模型共享了 TFWaveFormer 的配置。3 epochs 的结果还很初步，主要是看方法能不能 work，还远谈不上可靠的对比。
